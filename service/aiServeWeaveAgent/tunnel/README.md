...
**Agent 自动升级的 CHECK/UPGRADE/ROLLBACK 触发（STATUS.md P2「Agent 自动升级」子任务一）与模型拉取、ComfyUI Managed 走同一种形状，且比两者都更硬地撞上本文件头部的两条红线。** 一次升级最终会替换 Agent 自己正在执行的二进制，这个载荷比模型文件或容器镜像更敏感——它是将来会被 exec 的代码本身，因此这条消息比 `ModelPullTrigger`/`ComfyUIManagedAction` 更不能表达"去这个 URL 下载"或"跑这段指令"。`GatewayControl` 新增 `agent_upgrade_action`（`AgentUpgradeAction{action, target_version}`，`action` 是 CHECK/UPGRADE/ROLLBACK 三值封闭枚举），`AgentControl` 新增 `agent_upgrade`（`AgentUpgradeReport{current_version, state, reason, updated_unix_ms}`，单条状态而非 `repeated`——一个 Agent 只有一个正在运行的版本）。`AgentUpgradeAction` 只携带目标版本号，从不携带下载地址或签名：Agent 对着自己本地清单（`service/aiServeWeaveAgent/agentupgrade`，结构类比 `modelpull.Spec`）解析，版本号→URL→签名这条映射永远只活在 Agent 本地。子任务一只实现 CHECK：`agentupgrade.Checker` 把清单与自己的 `main.version` 比较；UPGRADE 与 ROLLBACK 在协议上被接受，但一律答复 `AGENT_UPGRADE_STATE_FAILED`/`AGENT_UPGRADE_FAILURE_REASON_NOT_IMPLEMENTED`，不下载、不签名校验、不重启——那是后续子任务的范围（子任务拆分见设计文档）。触发同样遵循 `GatewayControl_Config`/`GatewayControl_ModelPullTrigger` 的既有先例：不设专门 ack 帧，成功与失败都通过随后的 `AgentUpgradeReport` 观察。详见设计文档 [`docs/superpowers/specs/2026-09-19-p2-agent-auto-upgrade-design.md`](../../../docs/superpowers/specs/2026-09-19-p2-agent-auto-upgrade-design.md)。

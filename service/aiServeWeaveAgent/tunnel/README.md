...
| `RERANK` | `RerankRequest` | 单个 `DataChunk`（`RerankResponse`） |
...
**模型拉取的按名字触发（STATUS.md P2「模型分发」子任务二）没有走 Operation/`Serve` 槽，走的是 Control 流。** 模型拉取不属于任何一个 `runtime_id`——它是节点级的本地文件操作，不是某个推理运行时实例的请求——而每一次 `Serve` 槽派发都要求 `RequestHeaders.runtime_id` 命中 Agent 本地白名单，勉强塞一个假的 `runtime_id` 只会让以 runtime 为单位的调度器、槽池、熔断器全部对不上语义。`GatewayControl` 新增 `model_pull_trigger`（`ModelPullTrigger{names}`），`AgentControl` 新增 `model_pull`（`ModelPullReport{pulls: []ModelPullStatus}`），与 `RuntimeConfig`/`RuntimeStatus` 走的是同一条已经存在的、节点级的信道。`ModelPullTrigger` 只携带名字，从不携带 URL——Agent 对着自己本地清单（`service/aiServeWeaveAgent/modelpull`，子任务一）解析，Gateway 只能从 Agent 已经批准的名字集合里选，这是本文件头部「不得表达 fetch 这个 URL」这条红线在 Control 流上的延续，也是子任务一文档已经论证过的、`runtime_id` 白名单同一威胁模型的延伸。`ModelPullStatus.reason` 是封闭枚举（`ModelPullFailureReason`），从不携带原始错误文本——`Spec.SourceURL` 可能是带签名 token 的预签名 URL，Go 的 `http.Client` 会把完整请求 URL 编进传输错误文本，原样转发就会把这个细节带出 Agent。触发遵循 `GatewayControl_Config` 的既有先例：不设专门 ack 帧，成功与失败都通过随后的 `ModelPullReport` 观察。详见设计文档 [`docs/superpowers/specs/2026-09-17-p2-model-distribution-subtask2-design.md`](../../../docs/superpowers/specs/2026-09-17-p2-model-distribution-subtask2-design.md)。

**ComfyUI Managed 的生命周期动作触发（STATUS.md P2「ComfyUI Managed Docker 部署」子任务二）与模型拉取走同一种形状，但收窄的方式不同。** 子任务一原本设想"控制面下发部署规格"（镜像名、GPU 设备、挂载路径），但那正是本文件头部「不得表达 run this command」这条红线要挡住的东西——一个部署规格本质上就是"用这些参数跑这个镜像"的指令，比模型拉取子任务二当初设想的"下发 URL"更直接地撞线。因此收窄为与 `ModelPullTrigger` 同一种"只按名字/动作触发，规格留在本地"模式：`GatewayControl` 新增 `comfyui_managed_action`（`ComfyUIManagedAction{action}`，`action` 是 START/STOP/RESTART 三值封闭枚举），`AgentControl` 新增 `comfyui_managed`（`ComfyUIManagedReport{instances: []ComfyUIManagedStatus}`）。`ComfyUIManagedAction` 不携带容器名——今天一个 Agent 进程最多管理一个 Managed 实例，动作总是对着 Agent 本地已声明的那一个 Spec（`service/aiServeWeaveAgent/comfyuimanaged`，子任务一）执行，镜像、GPU、挂载路径永远不跨隧道；运维想换镜像版本，仍然要先改本机 flag 再触发 RESTART，从不是把镜像名交给 Gateway 转发。`ComfyUIManagedReport` 报告的是容器生命周期状态（pending/starting/running/stopped/failed），与已经通过既有 `RuntimeStatus`/`RuntimeSnapshot` 通道汇报的 ComfyUI 协议健康状况是两条互补而非重复的信道——一个 Managed 实例注册进 `runtime.Manager` 之后，它的健康状况走的仍然是 External 实例同一条既有路径，这条新信道只覆盖注册之前、或取消注册之后 `RuntimeStatus` 看不到的空档。触发同样遵循 `GatewayControl_Config`/`GatewayControl_ModelPullTrigger` 的既有先例：不设专门 ack 帧，成功与失败都通过随后的 `ComfyUIManagedReport` 观察。详见设计文档 [`docs/superpowers/specs/2026-09-18-p2-comfyui-managed-docker-subtask2-design.md`](../../../docs/superpowers/specs/2026-09-18-p2-comfyui-managed-docker-subtask2-design.md)。

**Agent 自动升级的 CHECK/UPGRADE/ROLLBACK 触发（STATUS.md P2「Agent 自动升级」）与模型拉取、ComfyUI Managed 走同一种形状，且比两者都更硬地撞上本文件头部的两条红线。** 一次升级最终会替换 Agent 自己正在执行的二进制，这个载荷比模型文件或容器镜像更敏感——它是将来会被 exec 的代码本身，因此这条消息比 `ModelPullTrigger`/`ComfyUIManagedAction` 更不能表达"去这个 URL 下载"或"跑这段指令"。`GatewayControl` 新增 `agent_upgrade_action`（`AgentUpgradeAction{action, target_version}`，`action` 是 CHECK/UPGRADE/ROLLBACK 三值封闭枚举），`AgentControl` 新增 `agent_upgrade`（`AgentUpgradeReport{current_version, state, reason, updated_unix_ms}`，单条状态而非 `repeated`——一个 Agent 只有一个正在运行的版本）。`AgentUpgradeAction` 只携带目标版本号，从不携带下载地址或签名：Agent 对着自己本地已签名清单（`service/aiServeWeaveAgent/agentupgrade`，结构类比 `modelpull.Spec`）解析，版本号→URL→签名这条映射永远只活在 Agent 本地。**子任务一**实现了 CHECK：`agentupgrade.Checker` 把清单与自己的 `main.version` 比较。**子任务二**补上了 UPGRADE 的完整链路：`target_version` 未命中清单直接答复 `AGENT_UPGRADE_FAILURE_REASON_UNKNOWN_VERSION`（不发起网络请求）；命中则依次经过 `AGENT_UPGRADE_STATE_DOWNLOADING`（下载到本地工作目录）→`AGENT_UPGRADE_STATE_VERIFYING`（对照清单签名已验证的 SHA256 校验，不过则 `AGENT_UPGRADE_FAILURE_REASON_VERIFICATION_FAILED`）→`AGENT_UPGRADE_STATE_DRAINING`（新增 `tunnel.Manager.DrainAll`，跨这个 Agent 持有的**每一条**隧道连接排空，方向与 Gateway 主动下发的单连接排空相反）→`AGENT_UPGRADE_STATE_RESTARTING`（`syscall.Exec` 自替换进程镜像，失败报告 `AGENT_UPGRADE_FAILURE_REASON_EXEC_FAILED`；成功则进程镜像已经替换，不会再有旧进程发出的后续报告）。ROLLBACK 仍一律答复 `AGENT_UPGRADE_STATE_FAILED`/`AGENT_UPGRADE_FAILURE_REASON_NOT_IMPLEMENTED`——那是子任务三的范围。触发同样遵循 `GatewayControl_Config`/`GatewayControl_ModelPullTrigger` 的既有先例：不设专门 ack 帧，成功与失败都通过随后的 `AgentUpgradeReport` 观察；一次 UPGRADE 进行中收到的第二次触发被静默忽略。详见设计文档 [`docs/superpowers/specs/2026-09-19-p2-agent-auto-upgrade-design.md`](../../../docs/superpowers/specs/2026-09-19-p2-agent-auto-upgrade-design.md)。

`RERANK`（STATUS.md 的 P2）是「新增 `InferenceRuntime` 方法」这套破坏性变更模式的试点，`AUDIO_TRANSCRIBE` 随后复用了同一套模式。但它请求体是纯文本 JSON（一个 query 加一组 documents），不携带任何二进制体，因此形状与 `EMBED` 相同——单个 `RequestHeaders.payload`、无 `DataChunk` 请求体、跑在推理槽而非批量槽。同样没有任何适配器的 `Discover` 会发布 `CapabilityRerank`，这个 Operation 到达 Agent 后同样总是被能力门禁拒绝，直到某个部署确认后端支持。
...
- **长稳测试：`TestSoak`（`service/aiServeWeaveGateway/e2e/soak_test.go`），时长与采样周期由环境变量驱动。** `AISW_SOAK_DURATION` 启用，`AISW_SOAK_SAMPLE_INTERVAL` 默认 5m，`AISW_SOAK_OUTPUT` 指定新的 CSV 路径，并生成 `.summary.json` 最终摘要。P10 已补正值与样本上限校验（最多 10,000）、失败清理、防覆盖、首尾采样与墙钟间隔检查；摘要记录后端来源、实际观测窗口、请求失败、资源增长和连接/槽范围。使用单进程三副本、真实回环 TCP/mTLS 与固定响应的**合成后端**，不是实际 Ollama 模型流量。前四分之一样本算术均值为资源基线，协程/堆末值增长上限仍为 100%，失败率上限仍为 1%；连接每次采样为 3、推理空闲槽合计在 `[1,12]`，采样间隔超过配置两倍判连续性不足。2026-09-12 的 2m/race 冒烟与 CSV/JSON 已归档，24h 冻结二进制运行已启动，须在结束后按退出码、日志和摘要核验，不能从启动记录推断已经通过或仍在运行。复现与完整边界见 [P10 验收手册](../../../deploy/p10-acceptance.md)、[本轮验收记录](../../../docs/acceptance/p10-2026-09-12/README.md)。

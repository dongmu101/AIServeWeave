...
9. **可查询的 Job 历史已经建好，但 Console 还没有页面接它，取消与产物访问仍然没有。** `GET /admin/v1/jobs/history` 与 `GET /admin/v1/jobs/history/:id`（J07，见上面「Job 持久化契约」一节的「已实现的持久化历史查询」小节）直接读 `jobs` 表，按租户、时间、状态、工作流分页，副本重启不再让它消失——`/admin/v1/jobs`（实时聚合）与它是两条并存的路径，回答的是两个不同的问题，不是谁取代谁。缺的是 Console 侧的页面（`app/console/jobs` 目前只渲染实时视图，尚未接这两个新端点）与取消/产物访问：后两者需要一条全新的「会话到租户级 Gateway 调用权限」链路，尚未设计，且明确不应该做成经由控制面转发已有的取消/产物直连路径（见上面同一小节的说明）。产物本身仍由 Gateway 数据面用租户的 API Key 提供，本服务不在那条路径上，控制台目前只能列出产物 id。

...
**未做及理由**：C26 持久化那部分原本卡在的后端能力——控制面 jobs 表、Gateway 写生命周期——已随根 STATUS 的 J01–J07（只读部分）落地：`GET /admin/v1/jobs/history` 与 `GET /admin/v1/jobs/history/:id` 现在可以按租户/时间/状态/工作流分页查询持久化历史，会话守卫、不依赖 Fleet 配置。但本控制台尚未接这两个新端点（仍只渲染 C25/C26 的实时视图），且取消与产物下载依旧未做。方案已定，见 ControlPlane README「Job 持久化契约」一节：本控制台服务端直接持有一把该租户的 Gateway API Key（owner/admin 用已有的「创建 API Key」功能生成，粘贴进本控制台一个新的设置页，由本控制台加密存储），不做 scope 收紧，取消与产物下载复用这把 Key 直连 Gateway 数据面，授权判断完全由本控制台自己的会话/角色体系承担；Gateway 与控制面都不需要为此改代码。已知代价是这把 Key 权限与用户自建 Key 相同，被拿到后不只能取消/读产物、也能拿去跑推理烧配额——这是当前没有真实租户在生产环境运行阶段的刻意权衡，不是遗漏，有真实租户后应重新评估是否收紧。尚未实现，不是接几个页面就能绕过的缺口。C27–C29——需要时序库与可检索日志存储，仓库里都没有，Prometheus 文本导出不等于历史曲线。

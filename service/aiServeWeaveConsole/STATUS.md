...
M1–M3、M4 的只读部分与 M5 的 C25/C26（含持久化历史与产物预览）已完成（见各节勾选项与验收记录），C27–C29 仍待后端能力，C24 的路由管理写路径已交付，见下方 P02 验收。M4/M5 不阻塞首版管理闭环；没有数据来源的菜单暂不开放，不用示例数据冒充运行状态。

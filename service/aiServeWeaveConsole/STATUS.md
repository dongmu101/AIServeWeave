...
**未做及理由**：C26 持久化那部分原本卡在的后端能力——控制面 jobs 表、Gateway 写生命周期——已随根 STATUS 的 J01–J07（只读部分）落地：`GET /admin/v1/jobs/history` 与 `GET /admin/v1/jobs/history/:id` 现在可以按租户/时间/状态/工作流分页查询持久化历史，会话守卫、不依赖 Fleet 配置。但本控制台尚未接这两个新端点（仍只渲染 C25/C26 的实时视图），且取消与产物下载依旧未做：产物由 Gateway 数据面用租户 API Key 提供，本控制台不持有那个 Key，取消同理需要一条本控制台目前没有的「会话到租户级 Gateway 调用权限」链路——这条链路的设计取舍见 ControlPlane README「Job 持久化契约」一节，尚未拍板，不是接几个页面就能绕过的缺口。C27–C29——需要时序库与可检索日志存储，仓库里都没有，Prometheus 文本导出不等于历史曲线。

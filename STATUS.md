...
历史验证索引：真实 Ollama + mTLS + Gateway 非流式/SSE、多副本联调、故障注入与滚动升级记录见 [隧道 README](service/aiServeWeaveAgent/tunnel/README.md) 和 Gateway e2e 测试。`service/aiServeWeaveGateway/e2e/TestSoak` 长稳工具仍在代码库中可用（合成后端回归冒烟），但真实环境下的长时间稳定性验证已改由维护者在自行部署的服务器上进行，不再要求在本仓库归档 24h 结果，见 [P10 验收手册](deploy/p10-acceptance.md)。
...
| [x] | P05 | 用户与会话生命周期 | Redis 权威会话已落地：JWT 强制随机 `sid`，旧 JWT 升级即失效；每账户最多 20 会话，支持当前/全部撤销与 30 秒 nonce 登录门，Redis 未配置拒绝启动、运行故障 fail closed 为 503。Compose 使用具名卷 + AOF `appendfsync always`，真实 Redis 验证活动会话与撤销均跨重启保持。租户用户支持自助改密，owner 可重置他人密码、改角色、禁用/启用及撤销会话；平台运维有独立对等管理面。数据库事务/行锁守住最后有效 owner/平台运维，禁用用户同事务吊销其创建的 active Key，重新启用不恢复；密码/角色变化不影响 Key。控制面 Key 缓存改为 generation 门控，避免旧数据库读取在吊销后回填；Gateway 本地缓存由 P06 的同一 generation 主动失效。Console 新增 `/console/account`、用户行管理、`/operator/operators` 与 `/operator/account`，退出先服务端撤销再清对应 Cookie，失败保留供重试。Go 全量格式/vet/build/generate/test/race、Console lint/TS7/134 测试/Webpack 构建、真实 PostgreSQL 18/MySQL 9.7 并发三连与 Chrome 双会话页面验收通过。 |
| [x] | P06 | Key 吊销通知 | ControlPlane 用 Redis Lua 原子 `INCR + PUBLISH` 推进校验 generation，并提供 `InternalToken` 守卫、2 秒心跳的长轮询；Gateway 启动时默认禁用正向缓存，同步 generation 后才启用，变化时整代清空，通知传输/超时/鉴权/状态码/格式异常时同步清空并停用缓存、逐请求回源校验，静默断线由 5 秒硬超时发现。在途旧验证由本地 epoch 阻止回填；漏 Pub/Sub、重连、控制面副本切换及 Redis generation 回退均由当前游标补偿。测试覆盖真实 Redis 双订阅者、漏消息与断连、真实 go-zero 路由、畸形/超大响应、静默超时、在途竞争、两 Gateway 1 秒传播和用户禁用闭环；`gofmt`、`go vet ./...`、`go build ./...`、proto 生成一致性、`go test ./...`、`go test -race ./service/...` 全部通过。数据库提交后、Redis 脚本前的崩溃补发由 P07 事务 outbox 承接；已通过鉴权的在途推理不被中断。 |
| [x] | P07 | 数据库升级与恢复 | 基础表以 PostgreSQL/MySQL 固定版本 SQL 替换 AutoMigrate；已有 Job/路由/模板保留 SQL 与版本号，共用专用连接锁、SHA-256 和 dirty 账本。独立 `-migrate up/status/resume` 不依赖 Redis/管理密钥，默认启动检查版本、列容量、完整主键、索引、InnoDB 与 outbox。事务 outbox 同事务保存吊销、每秒重试并在重启后补发，存储为一行有界合并。真实 PostgreSQL 18/MySQL 9.7 + Redis 8 的旧数据/旧账本升级、部分 DDL、并发锁、失败恢复、结构漂移拒绝、原生备份恢复逐表一致及生产补发/超时均通过 race 验证；全量 gofmt/vet/build/generate/test/race 门禁及审查通过，CI 已接入双数据库原生恢复。MySQL dirty 需检查后显式 resume；不提供破坏性 down，回退走已验证备份。操作步骤见 [数据库升级与恢复](deploy/database-recovery.md)，完整平台灾备仍属 A02。 |
| [x] | P08 | 指标、追踪与历史监控 | Registry 新增 `-metrics-addr` 与全部 RPC 的指标记录，控制面新增 `-metrics-addr`、给每条路由套上请求量/耗时中间件、记录吊销 outbox 滞后量与 Fleet/Registry 客户端调用结果，两处标签基数均有可执行的封闭取值测试。跨服务 trace 走最轻的路径：新增共享 `common/reqid` 包，把 Gateway 前门铸造的 request_id 一路带到 Scheduler 派发决策、Tunnel 分发/完成与 Agent 后端调用的结构化日志，不新增存储。历史时序复用现有关系库而非独立 TSDB：`common/metrics` 新增 `ParseExposition`(现有导出器的逆操作，零新依赖)，控制面新增 `internal/metricshistory` 定时抓取 Gateway/Registry 的 `/metrics` 文本、按标签白名单聚合(`node_id` 等高基数标签在写入历史前丢弃)、写入按 P07 固定版本 SQL 迁移落地的 `metrics_history_points` 表(5 分钟粒度，默认保留 90 天，另有独立保留期清理协程)，`GET /operator/v1/metrics/history` 按平台会话守卫查询并按 (metric, labels) 分组返回。Console 新增 `/operator/metrics`(C27)：请求量、成功率、延迟 p95 近似值、Token 用量、容量五组 ECharts 曲线，`dataZoom` 可交互，空态与错误态分离；是平台运维视角，不按租户拆分。过程中顺带修了一处真实缺陷：`common/metrics.Registry` 的 `Counter`/`Gauge`/`Histogram` 此前对 nil 接收者(一个未初始化的 `*Registry` 字段被装进 `runtime.Metrics` 接口)会 panic，已改为返回丢弃仪器。全量 `gofmt`/`go vet`/`go build`/proto 生成一致性/`go test ./...`/`go test -race ./service/...` 通过，`internal/store/gormstore` 的新增/相关用例已用真实 PostgreSQL 18 与 MySQL 9.7 逐一验证；Console 侧 `lint`/`typecheck`/`test`(145 用例)/`build` 通过，`/operator/metrics` 路由已确认可正常解析(未做需要真实平台会话与已采集数据的完整交互式浏览器验收)。 |
| [x] | P09 | 请求检索与告警 | **C28（请求与错误检索）已实现并测试**：新表 `request_logs`（PostgreSQL/MySQL 双支持，`gormstore/requestlogmigrate.go` 固定版本迁移）保存四个 OpenAI 前门端点（chat/responses/embeddings/models）里已通过鉴权请求的脱敏元数据——`request_id` 主键、`tenant_id`、`apikey.Display` 展示形式、封闭 `endpoint`/`outcome` 枚举、状态码、耗时、创建时间，不含请求体/响应体/模型名/node_id/Prompt/鉴权头。Gateway 新增 `requestLogMiddleware`（插在 `auth.middleware` 之后、`rateLimit` 之前）按状态码映射出 9 类封闭 outcome，经有界 channel（默认容量 10000、批大小 500、5 秒 flush）异步批量推送，缓冲满时丢弃并计入 `gateway_request_log_dropped_total`，推送失败（网络故障或控制面非 200）计入 `gateway_request_log_push_failed_total` 且不重试、不做无界缓冲。控制面新增 `POST /internal/v1/requestlogs`（`InternalToken` 守卫，按主键 `ON CONFLICT DO NOTHING` 天然幂等，独立 256 KiB 请求体上限——与 Gateway 批大小是耦合的跨服务约束，任一侧调整须核对另一侧，Gateway `controlplaneclient` 测试断言实际批次体积留有余量）与两个只读检索端点——`GET /admin/v1/requests`（会话守卫，按调用者租户过滤）、`GET /operator/v1/requests`（平台会话守卫，`tenant_id` 可选跨租户过滤）——共享 `since`/`until`（均可选，缺省表示该端不设边界）/`status`/`request_id`/`cursor`/`limit` 参数，复用既有 keyset 分页；除按租户查询的既有索引外另建 `(created_at, id)` 索引，供不限租户的运维跨租户检索与每日一次的保留期清理协程使用，避免全表扫描；保留期默认 30 天。Console 新增 `/console/requests`、`/operator/requests` 两个只读检索页。详见 [Gateway README「请求日志中间件与推送（P09/C28）」](service/aiServeWeaveGateway/README.md#请求日志中间件与推送p09c28)、[ControlPlane README「请求日志检索（P09/C28）」](service/aiServeWeaveControlPlane/README.md#请求日志检索p09c28)与设计文档 [`docs/superpowers/specs/2026-09-11-p09-request-search-design.md`](docs/superpowers/specs/2026-09-11-p09-request-search-design.md)。**C29（告警：计算、规则与处理状态）已实现并测试**：新表 `alert_rules`（阈值规则：`Metric`/`Operator`/`Threshold`/`ConsecutiveBuckets`/`WebhookURL`/`Enabled`，平台级不分租户）与 `alert_instances`（一次触发实例，`firing`/`acknowledged`/`resolved` 单向终态，与 Job 已有的终态约定一致），同样是 PostgreSQL/MySQL 双支持固定版本迁移（`gormstore/alertingmigrate.go`）。控制面新增 `internal/alertengine` 后台评估循环，随服务启动**无条件运行**（不像 `metricshistory` 那样需要先配置外部地址才启动，与 P09a 的请求日志保留期清理协程同一先例），默认每 60 秒一轮（`Config.AlertEvaluationInterval`，零值取 `config.DefaultAlertEvaluationInterval`），每轮读取全部 `Enabled=true` 规则，用新写的五个派生指标算法（`request_rate`/`success_rate`/`latency_p95`/`token_usage`/`capacity`，`internal/alertengine/metrics.go`）把 `metrics_history` 的原始 Prometheus 计数器/直方图桶折算成语义值——`success_rate`/`latency_p95` 是把此前只存在于 Console 前端 `lib/console/metrics-charts.ts`（`deltaByBucket`/`approxP95`）的近似算法移植成 Go 实现（同一套语义的独立重实现，两端无共享运行时），在规则的 `ConsecutiveBuckets` 窗口内比较阈值，按去抖动规则创建/维持/自动解除 `alert_instances`（已存在打开实例时只续 `LastEvaluatedAt`，不重复创建、不重复通知）。`alertengine/webhook.go` 的 `Sender` 在每次 firing/resolved 状态转换时按需投递一次 JSON payload：固定 3 次尝试、指数退避（1s/2s）、每次尝试 5 秒超时，全部失败后记录 `NotifyStatus=failed` 与已尝试次数，不重试、不入持久队列。控制面新增 5 个规则 CRUD 端点（`POST`/`GET`/`GET /:id`/`PATCH`/`DELETE` `/operator/v1/alert-rules`）与 2 个实例端点（`GET /operator/v1/alerts` 列表、`POST /operator/v1/alerts/:id/acknowledge` 确认处理），均挂 `requirePlatformSession`，写操作复用 `audit_logs`（`TenantID=model.PlatformScope`）。Console 新增 `/operator/alert-rules`（规则列表 + 创建/编辑/启停表单，`app/operator/alert-rules/alert-rules-view.tsx`）与 `/operator/alerts`（实例列表 + 确认处理按钮，`app/operator/alerts/alerts-view.tsx`）两个页面，挂在既有 `/operator` 导航、与 `/operator/metrics`（C27）相邻。详见 [ControlPlane README「告警评估与通知（P09/C29）」](service/aiServeWeaveControlPlane/README.md#告警评估与通知p09c29)与设计文档 [`docs/superpowers/specs/2026-09-11-p09-alerting-design.md`](docs/superpowers/specs/2026-09-11-p09-alerting-design.md)。Go 全量质量门禁（`gofmt`/`go vet`/`go build`/proto 生成一致性/`go test ./...`/`go test -race ./service/...`）与 Console `lint`/`typecheck`/`test`（157 用例）/`build` 全部通过；**未接入真实 PostgreSQL/MySQL** 跑 live 测试——`request_logs` 与新增 `alert_rules`/`alert_instances` 的双数据库迁移/CRUD 行为目前只由 `memstore` 与尚未执行的 `requestlogs_live_test.go`/`alerting_live_test.go` 覆盖，需要真实数据库或真实 Webhook 网关时另行验证，与 P02/P05/P08 等既有已勾选行按同一先例处理——核心实现与默认测试套件已完整通过即视为完成，真实数据库/浏览器交互验证作为已知遗留写入描述而不阻塞勾选。 |
| [x] | P10 | 稳定性与界面验收 | 长稳工具新增有界采样、CSV/JSON 摘要、防覆盖、失败清理与连续性检查；2m/race 冒烟及同版逐副本替换五次通过。Console Q03 已补 Chromium 75 项验收，并修复页面/卡片长文本溢出、短屏对话框、缺失标签和当前导航标记。真实流量熔断校准已完成：`service/aiServeWeaveGateway/main.go` 新增 `-breaker-failure-threshold`/`-breaker-base-cooldown`/`-breaker-max-cooldown` 三个 flag（默认沿用 `scheduler` 包内置值，零值不改变行为），2026-09-13 用它们在单机真实环境（ControlPlane + Registry + 真实 Agent + 真实 Ollama）对比了默认值（5/5s/2m）与一组更激进候选（3/2s/20s），覆盖稳定负载、中度/硬饱和与真实断连恢复；结论是保留当前默认值，详见[验收记录](docs/acceptance/p10-breaker-calibration-2026-09-13/README.md)。**真实环境下的 24h 长时间稳定性验证改由维护者在自行部署的服务器上进行，不再要求在本仓库归档结果**——`TestSoak` 工具本身保留在代码库中供合成后端回归冒烟使用。同版替换不代表跨版本兼容，版本与发版安排由维护者处理。操作步骤见 [P10 验收手册](deploy/p10-acceptance.md)，2026-09-12 的短时实测见[验收记录](docs/acceptance/p10-2026-09-12/README.md)。 |
...
- [Console STATUS](service/aiServeWeaveConsole/STATUS.md)：C29 与 Q03 待交付项；C22/C24/C27/C28 已交付；C25/C26 已交付范围与剩余限制。

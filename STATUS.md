...
| [ ] | P09 | 请求检索与告警 | **C28（请求与错误检索）已实现并测试**：新表 `request_logs`（PostgreSQL/MySQL 双支持，`gormstore/requestlogmigrate.go` 固定版本迁移）保存四个 OpenAI 前门端点（chat/responses/embeddings/models）里已通过鉴权请求的脱敏元数据——`request_id` 主键、`tenant_id`、`apikey.Display` 展示形式、封闭 `endpoint`/`outcome` 枚举、状态码、耗时、创建时间，不含请求体/响应体/模型名/node_id/Prompt/鉴权头。Gateway 新增 `requestLogMiddleware`（插在 `auth.middleware` 之后、`rateLimit` 之前）按状态码映射出 9 类封闭 outcome，经有界 channel（默认容量 10000、批大小 500、5 秒 flush）异步批量推送，缓冲满时丢弃并计入 `gateway_request_log_dropped_total`，推送失败（网络故障或控制面非 200）计入 `gateway_request_log_push_failed_total` 且不重试、不做无界缓冲。控制面新增 `POST /internal/v1/requestlogs`（`InternalToken` 守卫，按主键 `ON CONFLICT DO NOTHING` 天然幂等，独立 256 KiB 请求体上限——与 Gateway 批大小是耦合的跨服务约束，任一侧调整须核对另一侧，Gateway `controlplaneclient` 测试断言实际批次体积留有余量）与两个只读检索端点——`GET /admin/v1/requests`（会话守卫，按调用者租户过滤）、`GET /operator/v1/requests`（平台会话守卫，`tenant_id` 可选跨租户过滤）——共享 `since`/`until`（均可选，缺省表示该端不设边界）/`status`/`request_id`/`cursor`/`limit` 参数，复用既有 keyset 分页；除按租户查询的既有索引外另建 `(created_at, id)` 索引，供不限租户的运维跨租户检索与每日一次的保留期清理协程使用，避免全表扫描；保留期默认 30 天。Console 新增 `/console/requests`、`/operator/requests` 两个只读检索页。详见 [Gateway README「请求日志中间件与推送（P09/C28）」](service/aiServeWeaveGateway/README.md#请求日志中间件与推送p09c28)、[ControlPlane README「请求日志检索（P09/C28）」](service/aiServeWeaveControlPlane/README.md#请求日志检索p09c28)与设计文档 [`docs/superpowers/specs/2026-09-11-p09-request-search-design.md`](docs/superpowers/specs/2026-09-11-p09-request-search-design.md)。Go 全量质量门禁（`gofmt`/`go vet`/`go build`/proto 生成一致性/`go test ./...`/`go test -race ./service/...`）与 Console `lint`/`typecheck`/`test`（149 用例）/`build` 通过；**未接入真实 PostgreSQL/MySQL** 跑 live 测试，`request_logs` 的双数据库主键冲突/迁移行为目前只由 `memstore` 与尚未执行的 `requestlogs_live_test.go` 覆盖，需要真实数据库时另行验证。**C29（告警：计算、规则与处理状态）未实现**，仅有设计（同目录 `2026-09-11-p09-alerting-design.md`），后端告警 API 与 Console 页面均留待后续任务。 |
